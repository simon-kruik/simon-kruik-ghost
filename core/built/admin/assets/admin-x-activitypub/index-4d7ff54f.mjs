import { v as q, f as Q, u as K, a as C, b as tt, c as E, d as T, e as et, g as it, C as st, h as X } from "./index-8f5e89cf.mjs";
...
function ht(n, t, e, i) {
  ...
  const r = [], a = [], o = [], l = [], d = {}, p = (c) => `${c}:${d[c] = (d[c] ?? -1) + 1}`;
  let f = "", m = !1, g = !1;
  for (const c of s) {
    f += c.value;
    const h = c.type === "minusSign" || c.type === "plusSign" ? "sign" : c.type;
    h === "integer" ? (m = !0, a.push(...c.value.split("").map((x) => ({ type: h, value: parseInt(x) })))) : h === "group" ? a.push({ type: h, value: c.value }) : h === "decimal" ? (g = !0, o.push({ type: h, value: c.value, key: p(h) })) : h === "fraction" ? o.push(...c.value.split("").map((x) => ({
      type: h,
      value: parseInt(x),
      key: p(h),
      pos: -1 - d[h]
...
const ct = String.raw, dt = (() => {
  ...
  return !0;
})(), pt = typeof CSS < "u" && CSS.supports && CSS.supports("line-height", "mod(1,1)"), u = typeof matchMedia < "u" ? matchMedia("(prefers-reduced-motion: reduce)") : null, R = "--_number-flow-d-opacity", N = "--_number-flow-d-width", $ = "--_number-flow-dx", M = "--_number-flow-d", ut = (() => {
  try {
...
class z extends mt {
  ...
  set data(t) {
    ...
    if (this.created) {
      const l = this._data;
      this._data = t, this.computedTrend = typeof this.trend == "function" ? this.trend(l.value, o) : this.trend, this.computedAnimated = H && this._animated && (!this.respectMotionPreference || !(u != null && u.matches)) && // https://github.com/barvian/number-flow/issues/9
      rt(this), (e = this.plugins) == null || e.forEach((d) => {
        var p;
        return (p = d.onUpdate) == null ? void 0 : p.call(d, t, l, this);
...
var W;
const vt = parseInt((W = q.match(/^(\d+)\./)) == null ? void 0 : W[1]), L = vt >= 19, _t = [
  "data",
  "digits"
];
class F extends z {
...
class wt extends st {
  ...
  render() {
    const [t, { innerRef: e, className: i, data: s, willChange: r, isolate: a, group: o, digits: l, onAnimationsStart: d, onAnimationsFinish: p, ...f }] = D(this.props);
    return (
...
const Rt = /* @__PURE__ */ Q(function({ value: t, locales: e, format: i, prefix: s, suffix: r, ...a }, o) {
  ...
  const p = E(() => e ? JSON.stringify(e) : "", [
    e
  ]), f = E(() => i ? JSON.stringify(i) : "", [
    i
  ]), m = E(() => {
    var w;
    const g = U[w = `${p}:${f}`] ?? (U[w] = new Intl.NumberFormat(e, i));
    return ht(t, g, s, r);
  }, [
    t,
    p,
...
const bt = () => X(() => () => {
}, () => H, () => !1), St = () => X((n) => (u == null || u.addEventListener("change", n), () => u == null ? void 0 : u.removeEventListener("change", n)), () => u.matches, () => !1);
...
//# sourceMappingURL=index-4d7ff54f.mjs.map
